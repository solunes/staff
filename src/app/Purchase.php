...
class Purchase extends Model {
    ...
    public function purchase_products() {
        return $this->hasMany('Solunes\Staff\App\PurchaseProduct', 'parent_id');
    }

    public function getTotalAttribute(){
    	$total = 0;
    	$currency = \Solunes\Staff\App\Currency::first();
    	if(count($this->purchase_products)>0){
	    	foreach($this->purchase_products as $batch){
	    		$total += \Staff::calculate_currency($batch->total, $currency, $batch->currency);
	    	}
    	}
