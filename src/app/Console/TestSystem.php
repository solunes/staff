...
namespace Solunes\Staff\App\Console;
...
class TestSystem extends Command
{
    ...
    public function handle(){
        if(\App::environment('local')){
            ...
            $items = \Solunes\Staff\App\FieldTranslation::where('label', 'like', '%fields.%')->groupBy('label')->orderBy('label')->get();
            if(count($items)>0){
                $this->info('-----------------------------------------------');
            ...
            $items = \Solunes\Staff\App\FieldOptionTranslation::where('label', 'like', '%admin.%')->groupBy('label')->orderBy('label')->get();
            if(count($items)>0){
                $this->info('-----------------------------------------------');
            ...
            $items = \Solunes\Staff\App\MenuTranslation::where('name', 'like', '%admin.%')->groupBy('name')->orderBy('name')->get();
            if(count($items)>0){
                $this->info('-----------------------------------------------');
            ...
            $items = \Solunes\Staff\App\Field::whereIn('parent_id', $nodes_array)->where('type', 'text')->get();
            if(count($items)>0){
                $this->info('-----------------------------------------------');
